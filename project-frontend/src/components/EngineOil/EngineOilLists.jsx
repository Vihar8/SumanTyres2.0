...
import { deleteoilbyid, oils, togglestatusoils } from "../../api/common";
...
const AdminUserLists = () => {
  ...
  const tableHeading = [
      { id: "sNo", header: "S.No." },
      { id: "name", header: "Engine Oil Name" },
      { id: "type", header: "Bike/Activa" },
  ...
  const brandStatusUpdate = async () => {
      const response = await togglestatusoils({
      id: selectedBrandId,
      });
      if (response.statusCode == StatusCode.success) {
      setStatusUpdate(response?.oil);
      fetchBrandDetails();
  ...
  const handleDelete = async (_id) => {
      try {
      const response = await deleteoilbyid({ id: _id });
      if (response.statusCode === StatusCode.success) {
