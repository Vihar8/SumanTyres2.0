...
import { batterys, deletebatterybyid, togglestatusbatterys } from "../../api/common";
  ...
  const BatterysList = () => {
    ...
    const brandStatusUpdate = async () => {
        const response = await togglestatusbatterys({
        id: selectedBrandId,
        });
        if (response.statusCode == StatusCode.success) {
        setStatusUpdate(response?.batttery);
        fetchBrandDetails();
    ...
    const handleDelete = async (_id) => {
        try {
        const response = await deletebatterybyid({ id: _id });
        if (response.statusCode === StatusCode.success) {
    ...
    <React.Fragment>
       <>
        ...
        <Grid container item xs={12} className="headingSeparate">
            ...
            <Link className="mr-2" to={"/battery-add"}>
                <Tooltip title="Add Battery" placement="bottom" arrow>
                <Button
                    variant="contained"
                    color="success"
                    size="small"
